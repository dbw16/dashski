...
Backfilled advisories are the one exception, and a narrow one: they have no
fetch time to preserve, so they anchor on `issued_at` and are only ever written
earlier than any Snapshot we already hold (ADR 0017).
